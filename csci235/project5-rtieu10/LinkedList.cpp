...
//we want to get the current first ptr and check tht everything after is reversed
template<class T>
void LinkedList<T>::invertRest(Node<T>* current_first_ptr){
  //Node<T>* currentptr = current_first_ptr;
  if(current_first_ptr->getNext() == nullptr){  //keeps getting next until we are at the end of linked list
    //get next
    //setnext
    head_ptr_= current_first_ptr;   //set to head_ptr bc we we want to reverse the list, and the last pointer is now the beginning of the list
  }
  else{
    //current_first_ptr -> setNext(null);
    //current_first_ptr -> getNext()-> setNext(current_first_ptr);
    invertRest(current_first_ptr->getNext());   //keeps going until the end of the list
    current_first_ptr-> getNext() -> setNext(current_first_ptr);   //gets next, then sets the next value to the currentfirstptr
    current_first_ptr -> setNext(nullptr);   //we lose access to th
    //need to set the last element to setNext(null) bc the last elem would be set to null
...
void LinkedList<T>::invert(){
  if(head_ptr_ == nullptr){   //or if itemcount <= 1, just return
    return;
  }
  invertRest(head_ptr_);
}

template<class T>    //for loop in the else statement, for k times, base case: if k<=1 return? exit the if
void LinkedList<T>::rotate(int k){
  if(k==0){  //if the current ptr is at the end of the linked list
    //head_ptr_ = current_first_ptr; //cant do this because we lose access to the headpointer (1)
    return;

  }

  else{
    //the code below will rotate once, then pass k-1 through
    Node<T>* last = getNodeAt(item_count_ - 1); //sets last = to the last node
    Node<T>* last2 = getNodeAt(item_count_ - 2);  //sets the second to last to last2
    last->setNext(head_ptr_);  //links 4 -> 1 but  is still head_ptr
    head_ptr_ = last;   //sets head = to 4
    last2->setNext(nullptr);   //sets next for 3 to null
    rotate(k-1);
  }
